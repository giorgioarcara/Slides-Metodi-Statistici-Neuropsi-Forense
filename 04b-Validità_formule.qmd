...
Uno dei classici modi per vedere su tutti gli item di un test si comportano in maniera coerente è tramite l’utilizzo di metriche come l’alpha di Cronbach. 
...
\begin{columns}
  \column{0.3\textwidth}
    \begin{figure}
      \includegraphics[scale=0.25]{Figures/fattoriale3.png}
    \end{figure}
...
\begin{columns}
  ...
  \column{0.6\textwidth}
    \begin{figure}
      \includegraphics[scale=0.4]{Figures/fattoriale4.png}
    \end{figure}
...
Le analisi che si possono utilizzare (e come valutare i risultati) dipendono da se il criterio è una variabile continua (es. Il punteggio ad un altro test) oppure una variabile categoriale (es. L’appartenza ad un gruppo, es MCI vs Controlli).
