...
\begin{frame}{Tipi di affidabilità}
\phantomsection\label{tipi-di-affidabilituxe0}
\begin{itemize}
\tightlist
\item
...
Una formula molto diffusa per calcolare affidabilità test-retest,
soprattuto nei test sviluppati in Italiano, è quella della correlazione
di \emph{Pearson} (vedi anche l'Appendice di queste slides):
...
Il più grande problema dell'affidabilità test-retest quando misurata
tramite coefficiente di correlazione è che essa non cattura eventuali
\emph{effetti sistematici} di differenza tra i punteggi. Il valore
infatti ci dice quanto due valori sono correlati o consistenti, ma non
se sono più o meno gli stessi.
...
test-retest relibility se misurata tramite correlazione.

Se usata correlazione, sarebbe infatti necessario accompagnare da
analisi (es. t-tests) che permattano di vedere se c'è una differenza
...
Esistono però diversi modi di misurare affidabilità test-retest e sempre
più di recente, sono però utilizzate formule più appropriate (es. quelle
di Intraclass correlation che saranno trattate per affidabilità
inter-rater)
\end{frame}
...
test in Italino). In letteratura è però possibile trovare alcuni studi
che la utilizzano anche per test-retest e nel corso degli anni sempre
più seguono la più appropriata formula di ICC. In caso di effetto
pratica sarebbe infatti più adeguata.
\end{frame}

\section{Affidabilità split-half}\label{affidabilituxe0-split-half}
...
essere intrepretato come la percentuale di variabilità riconducibile al
...
anche l'affidabilità complessiva del test). (Vedi Urbina, 2004, per
esempio simile)
\end{frame}
...
Ogni coefficiente di affidabilità, in generale stimare la varianza
