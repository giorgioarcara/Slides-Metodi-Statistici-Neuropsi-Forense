...
\begin{frame}
% TITLE SLIDES
\title{Metodi Statistici per la Neuropsicologia Forense\\ \vspace{1em} \emph{5. Valutare i deficit e danni cognitivi}}
...
GH è un uomo di 68 anni, vedovo al centro di un contenzioso con una
compagnia di assicurazione. Al momento critico della nostra valutazione
ha appena avuto un trauma cranico in seguito ad un piccolo incidente
stradale, in cui però ha battuto la testa e perso conoscenza. L'unico
figlio sostiene che come conseguenza suo padre ha avuto un importante
peggioramento nel funzionamento cognitivo che si manifesta in incapacità
di comprendere bene ciò che gli viene detto. Nell'anamnesi di GH c'è
però un evento rilevante. Quando era più giovane (a 30 anni), sistemando
il tetto di un garage, GH è caduto da una scala e ha battuto la testa.
Andato in ospedale non è stato diagnosticato un trauma cranico, ma poco
dopo sono accaduti una serie di eventi: ha perso il lavoro che aveva da
tempo in una piccola azienda e ha divorziato con la moglie. La
neuropsicologia forense entra in gioco per due motivi. I consulenti per
l'assicurazione sostengono che il problema di GH è legato al trauma
cranico subito molti anni prima e quindi non legato all'incidente
stradale. Secondo i consulenti incaricati da GH invece il danno non è
riconducibile al precedente (e non documentato) trauma, ma solo a quello
più recente.
\end{frame}

\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi}
\emph{Quale sarebbe il primo scopo di una valutazione neuropsicologica
forense di GH?} \vspace{2em}

La maggior parte delle volte lo scopo di una valutazione
neuropsicologica ha come obiettivo valutare la presenza di un
deficit/danno cognitivo.
\end{frame}
...
In ambito forense potrebbe essere necessario valutare deficit o danno (o
entrambi).

Esempio: se lo scopo è valutare capacità di intendere o volere, potrebbe
essere importante valutare la presenza di deficit (non importa tanto
quale è l'eziologia, ma se la persona ha un livello che potrebbe
spiegare l'incapacità o capacità di intendere o volere).

Se lo scopo è valutare una perizia a fini assicurativi, sarebbe
importante valutare la presenza di un danno in seguito all'evento legato
all'assicurazione.

(la stima di un deficit ha utilità solo se ci dice che c'è stato un
peggioramento rispetto al livello precedente)
\end{frame}
...
La storia di GH è piuttosto complessa: alcuni elementi sembrano
suggerire che i disturbi siano insorti dopo il primo trauma cranico.

Ma il figlio sostiene che il padre sia peggiorato dopo il recente
incidente.

Quello che però ci interessa \textbf{non} è quali dei due eventi abbia
potuto causare un eventuale deficit o danno cognitivo, ma \textbf{se ci
sono elementi che possono spiegare il suo comportamento}.

È quindi più importante vedere il \emph{deficit} (piuttosto che il
\emph{danno}).
\end{frame}
...
di ottenere il punteggio 10 (quindi 75\% di risposte corrette). Cosa ci
dice questo punteggio?

\begin{center}
  \textbf{I punteggi ai test in sé sono poco informativi}
\end{center}

Per trarre informazioni utili, in neuropsicologia i punteggi si
confrontano spesso con dei dati di riferimento che ci aiutano a
classificare la prestazione.

Tra questidati spesso rivestono particolare rilevanza delle soglie
specifiche, chiamate \emph{cut-off}.
\end{frame}

\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-5}
\begin{center}
  \textbf{Cut-off e dati normativi}
\end{center}

I cut-off sono un punteggio-soglia al di sotto del quale la prestazione
è considerata deficitaria e (a seconda del contesto) il riflesso di un
danno cognitivo.

I cut-off vengono calcolati a partire dai dati normativi.

Spesso i cut-off sono riportati in tabelle, indipendentemente dal metodo
usato per calcolarli.
\end{frame}

\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-6}
\begin{center}
  \textbf{Esempio di cut-off riportati in tabelle}
\end{center}

\begin{center}
\includegraphics[width=0.5\linewidth,height=\textheight,keepaspectratio]{Figures/cutoff_humor.png}
\end{center}
...
Ci viene chiesto (in contesto clinico o forense) di effettuare una
valutazione neuropsicologica di un paziente con autismo grave coinvolto
in un reato.

\emph{Qual è in questo caso lo scopo della valutazione
neuropsicologica?}

Valutare se c'è deficit cognitivo che può avere influito sul commettere
il reato o, in generale classificare la sua performance e valutare le
sue abilità.
\end{frame}
...
Ci viene chiesto (in contesto clinico o forense) di effettuare una
valutazione neuropsicologica di un paziente con trauma cranico.

\emph{Quale è in questo caso lo scopo della valutazione
neuropsicologica?}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-14}
\begin{center}
...
\begin{center}
  **Sono tre diversi concetti di «normalità»**
\end{center}
\end{frame}

\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-15}
\begin{center}
  \textbf{Il concetto di normalità nei dati normativi}
\end{center}
...
nella vita quotidiana e in assenza di deficit cognitivi.}'' Ma come
definiamo se sono normali?
...
bias nei dati normativi o circolarità.
\end{frame}

\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-16}
\begin{center}
  \textbf{Tre aspetti fondamentali su cut-off e dati normativi (1)}
\end{center}
\vspace{2em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-17}
\begin{center}
  \textbf{Tre aspetti fondamentali su cut-off e dati normativi (2)}
\end{center}
\vspace{1em}
...
\textbf{Il fatto che una prestazione sia deficitaria o normale può avere significato o importanza diversa a seconda del contesto e della domanda.}
\vspace{1em}

\small
a) Nel caso di indagine di un disturbo dello sviluppo una performance al di sotto del cut-off è interessante in sé, indicandoci un deficit.

b) nel caso di sospetto di malattia o disturbo acuto nell’adulto viene usata per fare un’inferenza su un peggioramento dello stato cognitivo del paziente rispetto ad un evento che può avere una precisa collocazione temporale (es. ictus) oppure no (es. un deterioramento con un decorso legato alla malattia di Alzheimer).
\column{0.2\textwidth}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-18}
\begin{center}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-19}
\begin{center}
  \textbf{La raccolta dei dati normativi}
\end{center}

\begin{columns}
\column{0.5\textwidth}
\begin{figure}
\includegraphics[scale=0.20]{Figures/hist1.png}
\end{figure}
\begin{figure}
\includegraphics[scale=0.20]{Figures/hist3.png}
\end{figure}
\column{0.5\textwidth}

\begin{figure}
\includegraphics[scale=0.20]{Figures/hist2.png}
\end{figure}
\begin{figure}
\includegraphics[scale=0.20]{Figures/hist3_commentato.png}
\end{figure}
\end{columns}
\end{frame}

\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-20}
\begin{center}
  \textbf{Il significato del cut-off di normalità}
\end{center}
\vspace{2em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-21}
\begin{center}
...
Come potremmo definire la popolazione adeguata per un confronto normativo di GH?
Per GH sono gli italiani con età tra 65 e 70 anni?
Gli italiani che hanno esattamente 68 anni e che come GH hanno fatto solo le scuole elementari? Che lavorano come contadini?
\vspace{1em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-22}
\begin{center}
  \textbf{Una riflessione su cut-off e dati normativi}
\end{center}

\small

In questo caso l'adeguatezza del confronto può dipendere dal tipo di
domanda: \textbf{classificare la prestazione oppure stimare un danno}.

Se voglio \textbf{classificare GH rispetto ai suoi pari} (es. il suo
deficit non dipende dall'incidente) potrei semplicemente usare le
persone di 68 anni. Questo perché mi direbbe che, in generale, rispetto
alle persone con 68 anni ha prestazioni peggiori.

Se voglio \textbf{capire se GH ha sviluppato un danno} in seguito
all'incidente, allora dovrei usare i dati normativi quanto più simili al
suo stato nel momento in cui ha subito l'ipotetica lesione, per capire
se è avvenuto un cambiamento

\emph{Spesso, nonostante le diverse possibilità teoriche nel confronto
si sceglie il dato normativo più simile possibile all'individuo
valutato.}
\end{frame}

\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-23}
\begin{center}
  \textbf{Un altro aspetto fondamentale su cut-off e dati normativi}
\end{center}
\vspace{2em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-24}
\begin{center}
  \textbf{Un altro aspetto fondamentale su cut-off e dati normativi}
\end{center}
\vspace{2em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-25}
\begin{center}
...
mucchio ci insegna che le categorie a partire da una variabile continua
sono e saranno sempre arbitrarie.
\end{frame}

\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-26}
\begin{center}
  \textbf{Perché questo modo “contorto” di definire il deficit cognitivo e il danno?}
\end{center}
\vspace{2em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-27}
\begin{center}
  \textbf{Perché questo modo “contorto” di definire il deficit cognitivo e il danno? (1)}
\end{center}
\vspace{2em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-28}
\begin{center}
  \textbf{Perché questo modo “contorto” di definire il deficit cognitivo e il danno? (2)}
\end{center}
\vspace{2em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-29}
\begin{center}
  \textbf{Perché questo modo “contorto” di definire il deficit cognitivo e il danno? (2)}
\end{center}
\vspace{2em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-30}
\begin{center}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-31}
\begin{center}
  \textbf{Una nota sul definire il danno: Esempio 1}
\end{center}
\vspace{2em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-32}
\begin{center}
  \textbf{Una nota sul definire il danno: Esempio 2}
\end{center}
\vspace{2em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-33}
\begin{center}
  \textbf{Una nota sul definire il danno}
\end{center}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-34}
\begin{center}
  \textbf{Una nota sul definire il danno}
\end{center}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-35}
\begin{center}
  \textbf{Uno o più cut-off?}
\end{center}
...
Per un neuropsicologo clinico forense, laddove sia possibile a partire
dai metodi statistici a disposizione è comunque utile sapere la
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-36}
\begin{center}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-37}
\begin{center}
  \textbf{Metodi parametrici (base) per il calcolo dei cut-off}
\end{center}
\vspace{2em}
...
\begin{center}
\includegraphics[width=0.5\linewidth,height=\textheight,keepaspectratio]{Figures/gaussiana.png}
\end{center}
\emph{NOTA}: riferirci alla curva normale e alla trasformazione in punti
\(Z\) ci aiuta ad identificare percentili, cioè valori che delimitano
percentuali di dati.
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-38}
\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-39}
\begin{columns}
\column{0.8\textwidth}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-40}
\begin{center}
  \textbf{PARENTESI}
  
  \textbf{Standardizzare (trasformare in z-score) non rende la distribuzione normale!}
\end{center}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-41}
\begin{center}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-42}
\begin{center}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-43}
\begin{center}
  \textbf{Campione e Popolazione nei dati normativi}
\end{center}
\vspace{1em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-44}
\begin{center}
  \textbf{Variabili demografiche nei dati normativi}
\end{center}
\vspace{2em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-45}
\begin{center}
  \textbf{Variabili demografiche nei dati normativi}
\end{center}
\vspace{2em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-46}
\begin{center}
  \textbf{Variabili demografiche nei dati normativi}
\end{center}
\vspace{2em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-47}
\begin{center}
  \textbf{Variabili demografiche nei dati normativi}
\end{center}
\vspace{2em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-48}
\begin{center}
  \textbf{Quale è il metodo migliore per il calcolo dei cut-off?}
\end{center}
\vspace{2em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-49}
\begin{center}
  \textbf{Riepilogo su dati normativi e cut-off}
\end{center}
\vspace{1em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-50}
\begin{center}
  \textbf{Aspetti da considerare nell’utilizzo dei dati normativi}
\end{center}
\vspace{2em}

Per stimare correttamente la prestazione del mio soggetto (e rendere
sensato il confronto con i dati normativi), i dati normativi devono
essere rappresentativi.
\end{frame}

\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-51}
\begin{center}
  \textbf{Aspetti da considerare nell’utilizzo dei dati normativi}
\end{center}
\vspace{2em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-52}
\begin{center}
  \textbf{Aspetti da considerare nell’utilizzo dei dati normativi}
\end{center}
\vspace{2em}

\textbf{2) Specie per stimare il danno, utilizzare dati normativi non
necessariamente composti da grandi numeri di soggetti, ma simili
all'individuo da valutare.} \vspace{2em}

Un campione molto numeroso ma poco rappresentativo non è utile.
\end{frame}

\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-53}
\begin{center}
  \textbf{Aspetti da considerare nell’utilizzo dei dati normativi}
\end{center}
\vspace{1em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-54}
\begin{center}
  \textbf{Aspetti da considerare nell’utilizzo dei dati normativi}
\end{center}
\vspace{1em}
...
\begin{frame}{Valutare i deficit cognitivi}
\phantomsection\label{valutare-i-deficit-cognitivi-55}
\begin{center}
  \textbf{Ricapitolando}
\end{center}
\vspace{1em}

In neuropsicologia clinica e forense spesso dobbiamo valutare due
aspetti:

C'è un \textbf{deficit cognitivo} o stiamo vedendo il livello di abilità
cognitiva generale del paziente? \emph{Questo è fatto confrontando con
dati normativi e classificando (spesso con percentuali o sopra/sotto
norma)}
